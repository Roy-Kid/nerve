...
1. **One job per conversation** — id `{producer}:{session_id}`. Subagents refine main `current` only; no child session rows.
...
6. **Public docs** — edit `index-page/src/docs/content.ts` (and site UI), not a repo `docs/` folder. Keep root/plugin READMEs as short pointers.
