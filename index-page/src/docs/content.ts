...
export const docNav: DocNavItem[] = [
  ...
  },
  {
    slug: 'ingest',
    title: 'Ingest API',
    summary: 'Loopback HTTP for snapshots, events, demo, and actions.',
  },
  {
    slug: 'privacy',
    title: 'Privacy',
...
./scripts/verify_loop.sh`,
      ...
      {
        type: 'p',
        text: 'Settings tabs: General · Machines · Appearance · Notifications · About.',
      },
...
codex plugin add nerve@nerve`,
      ...
      },
      {
        ...
        items: [
          ...
          'Each row has Dismiss (local) — removes the panel row without signaling the agent.',
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'Open agent sessions stay on the panel while lifecycle is active. On SessionEnd the job is removed immediately. Shell / subagent still running is Running (never Attention). Monitor-only background work is Success green — a phase finished and the stream is waiting for feedback. Waiting for your next prompt is Attention. SessionStart is Ready/Inactive (not “waiting on you”). Closed terminals on this Mac are reaped via producer PID when present; otherwise use Dismiss.',
      },
      { type: 'h2', text: 'Main-session lifecycle' },
      {
        ...
        rows: [
          ...
          ['Waiting on you', 'idle · permission', 'Attention'],
      ...
      { type: 'h2', text: 'How a session leaves the panel' },
      {
        ...
        items: [
          ...
          'Dismiss action — user removes the row locally.',
        ],
      },
      { type: 'h2', text: 'Design notes' },
      {
        ...
        items: [
          ...
          'Out of scope: multi-display ribbons, cloud sync, disk history, in-app agent chat.',
        ],
      },
    ],
  },
  {
    slug: 'ingest',
    title: 'Ingest API',
    lede: 'Loopback only: http://127.0.0.1:17890. Remotes reach this port through SSH reverse tunnels.',
    blocks: [
...
curl -s -X POST 'http://127.0.0.1:17890/v1/actions/result?sourceId=my-source' \\
  ...
  -d '{"id":"<pending-id>","state":"succeeded","message":"ok"}'`,
      ...
      {
        type: 'p',
        text: 'Wire sample in the repo: fixtures/demo_snapshot.json.',
      },
    ],
  },
  {
